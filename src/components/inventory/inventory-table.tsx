...
import { Trash2, Edit3, PlusSquare, MinusSquare, ArrowUpDown, ArrowUp, ArrowDown, PackageOpen } from 'lucide-react';
...
type InventoryTableProps = {
  ...
  onAssignToContainer?: (item: StoredInventoryItem) => void;
};
...
export default function InventoryTable({ items, onDeleteItem, onEditItem, onRecordTransaction, onAssignToContainer }: InventoryTableProps) {
          ...
          <Table>
            ...
            <TableHeader className="sticky top-0 z-20 bg-background shadow-sm">
              <TableRow className="hover:bg-background">
                ...
                <TableHead className="text-center sticky right-0 bg-background shadow-[-2px_0_4px_rgba(0,0,0,0.05)] min-w-[100px] z-10">Aktionen</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              ...
              {sortedAndFilteredItems.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="sticky right-0 bg-background shadow-[-2px_0_4px_rgba(0,0,0,0.05)] z-10">
                    <div className="flex flex-col gap-1 items-center min-w-[80px]">
                      <div className="flex gap-1">
                        <TooltipProvider delayDuration={100}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" onClick={() => onRecordTransaction(item, 'Zugang')} className="h-7 w-7 text-green-600 hover:text-green-700">
                        ...
                        <TooltipProvider delayDuration={100}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" onClick={() => onRecordTransaction(item, 'Abgang')} className="h-7 w-7 text-orange-600 hover:text-orange-700">
                        ...
                        {onAssignToContainer && (
                          <TooltipProvider delayDuration={100}>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button variant="ghost" size="icon" onClick={() => onAssignToContainer(item)} className="h-7 w-7 text-purple-600 hover:text-purple-700">
                                  <PackageOpen className="h-3.5 w-3.5" />
                                </Button>
                              </TooltipTrigger>
                              <TooltipContent><p>In Container füllen</p></TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                        )}
                      </div>
                      <div className="flex gap-1">
                        <TooltipProvider delayDuration={100}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" onClick={() => onEditItem(item)} className="h-7 w-7 text-blue-600 hover:text-blue-700">
