...
export default function InventoryManagement() {
  ...
  const clearArtikelDefinitionen = () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('artikelDefinitionen', JSON.stringify([]));
    }
  };
  // LocalStorage: Lagerdaten beim Start laden
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const items = localStorage.getItem('inventoryItems');
      if (items) {
        try { setInventoryItems(JSON.parse(items)); } catch (err) {}
      }
      const artikel = localStorage.getItem('artikelDefinitionen');
      if (artikel) {
        try { setArtikelDefinitionen(JSON.parse(artikel)); } catch (err) {}
      }
      const transactions = localStorage.getItem('inventoryTransactions');
      if (transactions) {
        try { setInventoryTransactions(JSON.parse(transactions)); } catch (err) {}
      }
    }
  }, []);
  ...
  const handleDeleteArtikelDefinition = (definitionId: string) => {
    setArtikelDefinitionen(prevDefs => {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('artikelDefinitionen', JSON.stringify(updated));
      }
      return updated;
    });
    toast({
