...
- [ ] 🔜 **Aufgabe 17 – GFKC-Verschnitt-Buchung: ZURÜCKGESTELLT, Bestandsaufnahme + Fachkontext vollständig (26.09.2026, korrigiert 27.09.2026).** Start geplant für die Woche ab 29.09.2026. Fachkontext aus separatem Claude-Desktop-Projekt nachgeliefert und eingearbeitet: `docs/GFKC-FACHKONTEXT-REZEPTUR-2026-09.md` (Referenzrezeptur, Rechenmethodik, neue fix/reduzierbar-Komponentenlogik für GFKC-O nötig). Ziel-ABV 53,5 % ist ein **gelebter Richtwert, kein technisches Gate** — `zielAlkohol` bleibt frei editierbar, jede Charge durchläuft ohnehin denselben Freigabeprozess, Abweichung bedeutet nur Neuberechnung beim Lohnabfüller (ursprüngliche Annahme einer starren IFS-Sperre war falsch, korrigiert). **Nicht vorher implementieren.**

  📄 **Vollständige Bestandsaufnahme: `docs/GFKC-VERSCHNITT-BESTANDSAUFNAHME.md`**

  **Fachlicher Ablauf (bestätigt 26.09.2026):** Keine fixe Rezeptur, nur grobe Näherung → ausmischen → mit Einzelkomponenten nachjustieren → ABV am Ende einstellen (verdünnen/aufspriten) → **erst dann buchen** → GFKC lagern, tranchenweise Abgang (kein Rücklauf, im Gegensatz zu Lohnbrand).

  **Wichtigster Fund:** Auf Branch `pages-clean` (**selbes Repo**, nicht das vom Nutzer vermutete separate Projekt — daher die Verwirrung) existiert bereits ein vollständiges, gut durchdachtes Planungswerkzeug: `rezepturSchema.ts` (Komponenten in L/%, freie Zutaten wie Wasser, Alkoholkorrektur-Block, Sensorik-Bewertung, Status-Workflow `entwurf→test→freigegeben→produziert→archiviert`), `rezeptur-manager.ts` (321 Zeilen reine Berechnungs-/Skalierungs-/Validierungslogik), UI unter `/rezepturen` + `/rezepturen/editor` (1441 Zeilen). Die Alkoholkorrektur-Formel (Wasser verdünnen bei zu hohem ABV, Sprit aufspriten bei zu niedrigem) ist bereits fertig implementiert.

  **Die entscheidende Lücke:** Der "Produktionsmischung hergestellt"-Schalter ist nur eine Status-Checkbox — es gibt **keine einzige Funktion, die tatsächlich bucht**. Kein Aufruf einer Stock-Buchung, kein UI-Feld für Zieltank/Chargennummer, das dafür vorgesehene `produktionsDaten`-Schema-Feld wird nirgends befüllt. Genau das ist die Kernaufgabe für die Umsetzung — plus: Datenzugriffsschicht von `pages-clean`s zentraler `app-auto-sync.ts` auf das aktuelle `fresh-main`-Muster (dediziertes `rezeptur-service.ts` analog zu `stock-service.ts`/`lohnbrand-service.ts`) migrieren, LA-Bilanz-Kontrollrechnung ergänzen (sollte bei reinem Verschnitt ohne Destillation verlustfrei aufgehen), hartcodierten `spritStaerke = 60` durch echten Lagerbestand ersetzen.
- [ ] ⚠️ **Aufgabe 18 – Inventory/Tanks-Referenz reparieren (NEU, gefunden bei Aufgabe 13):** Teilweise erledigt. Die Aufgabe-4-Migration (Tank-ID-Normalisierung) hatte nur `tanks[]` in `tank-data.json` migriert, nicht `inventory[]`. **41 Inventar-Zeilen** verwiesen weiterhin auf alte Gruppen-Tanknummern (`Fass`: 6, `Fl`: 5, `B`: 25, `K`: 3, `Cont`: 1, `IBC`: 1).
  - ✅ **Erledigt (13 Zeilen):** `Fass`, `Fl`, `Cont`, `IBC` — jedes Gebinde dieser Gruppen hatte einen inhaltlich eindeutigen `produktName`/`currentContent`, automatisch per Namensabgleich (ASCII-normalisiert) korrigiert und verifiziert (0 verbleibende alte Referenzen in diesen 4 Gruppen).
  - ⚠️ **Offen (28 Zeilen):** `B` (25) und `K` (3) — mehrere Gebinde mit identischem Inhalt je Gruppe (z.B. 5× "Marc d SWS" in Gruppe B). Automatische Zuordnung wäre reine Vermutung — wird geklärt, sobald die physische Zuordnung der Gebinde vorliegt.
