...
- [ ] **Aufgabe 18 – Inventory/Tanks-Referenz reparieren (NEU, gefunden bei Aufgabe 13):** Die Aufgabe-4-Migration (Tank-ID-Normalisierung) hat nur `tanks[]` in `tank-data.json` migriert, nicht `inventory[]`. **41 Inventar-Zeilen** verweisen weiterhin auf alte Gruppen-Tanknummern (`Fass`: 6, `Fl`: 5, `B`: 25, `K`: 3, `Cont`: 1, `IBC`: 1) statt auf die jetzt eindeutige Tanknummer des jeweiligen Gebindes. Das betrifft nicht nur `tank-viewer.html`, sondern auch die Live-Zuordnung in `tank-management.tsx`/`tank-content-manager.tsx` (beide matchen exakt auf `tankNr`).
  - **Unproblematisch, da eindeutig zuordenbar (14 Zeilen):** `Fass`, `Fl`, `Cont`, `IBC` — jedes Gebinde dieser Gruppen hat einen inhaltlich eindeutigen `produktName`/`currentContent`, eine automatische Migration per Namensabgleich ist sicher möglich.
  - **⚠️ Nicht automatisch lösbar (27 Zeilen):** `B` und `K` — hier gibt es je Gruppe **mehrere Gebinde mit identischem Inhalt** (z.B. 5× "Marc d SWS" in Gruppe B). Eine automatische Zuordnung wäre reine Vermutung, welches konkrete Fass zu welcher Inventarzeile gehört — bei zoll-/buchungsrelevanten Daten nicht vertretbar ohne Bestätigung. Erfordert manuelle Zuordnung durch jemanden, der die physischen Gebinde kennt.
