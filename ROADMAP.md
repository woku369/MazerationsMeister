...
> ⚠️ **Gegenprüfung (September 2026):** Alle 9 Aufgaben wurden in VS Code als erledigt gemeldet und zunächst hier als `[x]` eingetragen. Eine anschließende, evidenzbasierte Prüfung gegen den tatsächlichen Code (nicht nur die Commit-Messages) hat gezeigt, dass diese pauschale Einstufung **nicht korrekt** war. Status unten korrigiert: 3 wirklich sauber erledigt (✅), 3 teilweise mit Substanzverlust (⚠️), 2 sachlich nicht erledigt trotz gegenteiliger Commit-Message (❌).
...
- [ ] ❌ **Aufgabe 4 – Tank-ID-Normalisierung an der Quelle:** **Nicht wirklich erledigt.** `fixTankIds()` wurde nur umbenannt (`syncTankDefinitionsWithInventory` in `tank-sync.ts`) und läuft mit identischer Logik weiterhin bei jedem Aufruf – keine einmalige Migration, sondern dieselbe Live-Reparatur unter neuem Namen. Gegenprobe an den echten Produktionsdaten: **41 von 50 Tanks in `tank-data.json` haben weiterhin `id !== tankNr`.** Die geforderte Anpassung von `tank-data.json` an ein bereinigtes Modell fand nicht statt. Einziger echter Fortschritt: das 7-fache OR-Fallback-Matching in `tank-viewer.html` wurde auf einfaches `id === tankId` reduziert – Symptom kosmetisch behandelt, Ursache weiterhin offen.
- [ ] ⚠️ **Aufgabe 5 – Formeln konsolidieren:** Teilweise – Inventory-Seite (`inventory-table.tsx`, `inventory-summary.tsx`, `tank-content-manager.tsx`, `inventory-management.tsx`, `sammelliste/page.tsx`) ruft jetzt tatsächlich `calcLA`/`toVolumeLiters` aus `mazeration-calc.ts` auf. Aber: Aufgabe 9 hat im selben Zug in `mazeration-form-helpers.ts` eine **neue, eigenständige** LA-Berechnung eingeführt, die die zentrale Funktion nicht nutzt – die Formel-Dopplung ist damit für das Hauptformular teilweise wieder da. PWA-Kommentar auf TS-Quelle korrekt umgesetzt.
...
- [ ] ❌ **Aufgabe 8 – Aufräumen (Quick Wins):** **Nicht wirklich erledigt**, Commit-Message übertreibt den Umfang deutlich. Tatsächlich gemacht: `webSecurity: true` in nur 2 von 4 Electron-Dateien (`main.js`, `main.ts` – `main-simple.js`/`main-simple.ts` weiterhin `false`), 7 leere Stub-Dateien + `tank-management-backup.tsx` gelöscht. **Nicht gemacht:** der namensgebende tote ~280-Zeilen-Codeblock (zweites, unerreichbares `return` in `inventory-management.tsx`, Zeile 963–1247) existiert unverändert; `let lastProduktName` modul-global weiterhin da (Zeile 4); `electron/main-corrected.js`/`main-fast.js`/`main-optimized.js` weiterhin vorhanden; `item.menge`-Bug in `github-service.ts` nicht gefixt; „Universal Storage"-Schicht weder entfernt noch angebunden.
- [ ] ⚠️ **Aufgabe 9 – Strukturelle Aufteilung:** Teilweise – Token-Konsolidierung (`src/lib/github-token.ts`) echt und vollständig umgesetzt (3 Stellen migriert). Aber „strukturelle Aufteilung" von `mazeration-form.tsx` fiel deutlich kleiner aus als beauftragt: nur 2381 → 2041 Zeilen (~14 % kleiner), PDF-/DOCX-/XLSX-Erzeugung bleibt weiterhin inline im Formular statt in eigenen Modulen. `inventory-management.tsx` (Ziel: Aufteilung) wurde von diesem Commit gar nicht angefasst.
