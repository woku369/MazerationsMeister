...
- [x] **Aufgabe 4 – Tank-ID-Normalisierung an der Quelle:** Invariante `id === tankNr` dokumentiert. `fixTankIds()` aus `tank-sync.ts` entfernt, Aufruf aus `tank-management.tsx` entfernt. 7-faches OR in `tank-viewer.html` auf `t.id === tankId` reduziert. Dual-Fallback in `use-app-data.ts` und `tank-offline/page.tsx` vereinfacht.
- [x] **Aufgabe 5 – Formeln konsolidieren:** `toVolumeLiters`, `calcLA`, `korrDichte20`, `calcVolumeFromMassAndDensity` in `mazeration-calc.ts`. Alle Inline-Duplikate in `inventory-table.tsx`, `inventory-summary.tsx`, `tank-content-manager.tsx`, `inventory-management.tsx`, `mazeration-form.tsx`, `sammelliste/page.tsx` ersetzt. PWA-Kommentar auf TS-Quelle. Tests in `src/lib/__tests__/mazeration-calc.test.ts`.
